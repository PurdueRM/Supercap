...
{
    duty = ClampFloat(duty, DUTY_FEEDFORWARD_MIN, DUTY_FEEDFORWARD_MAX);
    return (uint32_t)(duty * (float)htim->Instance->ARR);
}

static void PowerStage_ApplyVoltageRatioDuty(float vbus, float vcap)
{
    float d_bus = DUTY_FEEDFORWARD_CENTER;
    float d_cap = DUTY_FEEDFORWARD_CENTER;

    if ((vbus > 1.0f) && (vcap > 1.0f))
    {
        if (vbus >= vcap)
        {
            d_cap = DUTY_FEEDFORWARD_CENTER;
            d_bus = DUTY_FEEDFORWARD_CENTER * (vcap / vbus);
        }
        else
        {
            d_bus = DUTY_FEEDFORWARD_CENTER;
            d_cap = DUTY_FEEDFORWARD_CENTER * (vbus / vcap);
        }
    }

    d_bus = ClampFloat(d_bus, DUTY_FEEDFORWARD_MIN, DUTY_FEEDFORWARD_MAX);
    d_cap = ClampFloat(d_cap, DUTY_FEEDFORWARD_MIN, DUTY_FEEDFORWARD_MAX);

    __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, DutyToCompare(&htim1, d_bus));
    __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_4, DutyToCompare(&htim3, d_cap));
}

void PowerStage_SetPhaseSystem(float target_power, float control_effort)
{
    control_effort = ClampFloat(control_effort, 0.0f, 1.0f);
    global_control_effort = control_effort;

    PowerStage_ApplyVoltageRatioDuty(bus_voltage, cap_voltage);

    uint32_t tick_offset = (uint32_t)(control_effort * (float)MAX_PHASE_SHIFT_TICKS);
    ...
    if (target_power > 0.0f)
    {
        ...
        if (tick_offset < 1U) tick_offset = 1U;
    ...
    else if (target_power < 0.0f)
    {
        ...
        if (tick_offset >= HALF_DUTY_TICKS) tick_offset = HALF_DUTY_TICKS - 1U;
    ...
    else
    {
        ...
        global_phase = 0.0f;
        power_integral = 0.0f;
        __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_2, 1U);
    }
}
